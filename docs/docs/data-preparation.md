...
| `dim_customers` | Dimension | order (see note below) | `customer_id` | 99,441 |
...
Note on customers: Olist generates a **new `customer_id` for every order**, so `dim_customers`
has one row per order and is related one-to-one with `fact_orders`. The person is identified by
`customer_unique_id`, a column of `dim_customers` (96,096 people, 2,997 of them with more than
one order).
...
| 4 | `fact_orders[customer_id]` | `dim_customers[customer_id]` | one-to-one | One customer row per order |
...
| **`delivery_days`**, **`promised_days`** (purchase to delivery, purchase to estimated date; only for delivered orders) | Compare real delivery time with the promise |
| **`delay_days`** (delivery date minus estimated date, compared by day) and **`delivery_status`** (`On time` / `Late`) | The estimated date has no time, so comparing timestamps would count same-day deliveries as late |
| **`delivery_bucket`** (delivery time in five ranges) | Review score by delivery time |
| **`handling_days`**, **`shipping_days`** (purchase to carrier, carrier to customer) | Shows where the days go: seller or carrier |
...
"Remove duplicates" was tried first, but it does not respect the sort order (it kept an older
review), so the latest review is chosen with a group-by and a maximum.
...
| Single mapping list (exact match, one pass) to fix typos and merge near-duplicate categories (`Home Confort`, `Fashio Female Clothing`, `Costruction Tools`...) | Categories reduced to 45, so charts stay readable |
...
- Columns: `date`, `year`, `month`, `month_name`, `day`, `quarter`, `year_quarter`,
  `weekday_number` and `day_type` (weekday or weekend).
